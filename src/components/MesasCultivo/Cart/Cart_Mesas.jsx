...
class Cart extends React.Component {
    ...
    addNewItemToCart(data) {
        postData('carrito', data.mesas).then(() => this.props.guardada(data.mesas)).catch((error) => console.log(error));
    }
    ...
    create(data) {
        getData(`carrito/${data.mesas.id}`).then(function (response) {
            if (!Array.isArray(response)) {
                response.quantity++;
                this.addItemQtyToCart(response)
            } else {
                data.mesas.quantity = 1;
        ...
        return (
            <Col sm={4} md={4}>
                <Card id="carta" style={{ width: '14rem' }}>
                    <Card.Img variant="top" src={this.props.mesas.imagen} alt="242x200" />
                    <Card.Body>
                        <Card.Title><h4>{this.props.mesas.name}</h4></Card.Title>
                        <Card.Text>
                            <h6>{this.props.mesas.description}</h6>
                            <h2>$ {this.props.mesas.costo} .00</h2>
                        </Card.Text>
